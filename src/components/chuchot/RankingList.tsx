...
export function RankingList({ questions, onSelectQuestion }: RankingListProps) {
  return (
    <div className="bg-white rounded-2xl p-6 border-2 border-primary/5 shadow-xl overflow-hidden relative">
        ...
        {questions.map((q, idx) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0 border-b border-primary/5 pb-4 group-last:border-none">
              <p className="text-[15px] font-bold line-clamp-1 leading-snug group-hover:text-accent transition-colors mb-2">
                {q.title}
              </p>
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-black text-accent bg-accent/5 px-2 py-0.5 rounded-full uppercase">@{q.nickname}</span>
                <span className="flex items-center gap-1.5 text-[11px] font-bold text-primary/40">
      ...
      <button className="w-full mt-6 bg-primary text-accent hover:bg-primary/95 font-black text-sm py-3.5 rounded-2xl shadow-lg transition-all transform hover:scale-[1.02] active:scale-95">
        인기글 더보기
      </button>
    </div>
  )
