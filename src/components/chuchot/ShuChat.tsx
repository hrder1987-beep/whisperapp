...
import { Send, User, Maximize2, X } from "lucide-react"
...
const ChatMessage = memo(({ msg, activeBot, botIconUrl }: { msg: Message, activeBot: BotType, botIconUrl?: string }) => (
  <div className={cn("flex items-start gap-3", msg.role === "user" ? "flex-row-reverse" : "flex-row")}>
    <div className="shrink-0">{msg.role === "user" ? (<div className="w-8 h-8 rounded-full bg-black/5 flex items-center justify-center"><User className="w-4 h-4 text-muted-foreground" /></div>) : (<AvatarIcon src={botIconUrl} avatarId={activeBot} className="w-8 h-8 shadow-sm" />)}</div>
    <div className={cn("max-w-[85%] p-3.5 rounded-[1.2rem] text-[14px] leading-relaxed shadow-sm", msg.role === "user" ? "bg-accent text-primary rounded-tr-none" : "bg-white border border-black/[0.08] text-foreground rounded-tl-none")}>{msg.text}</div>
  </div>
));
  ...
  return (
    <div className="flex flex-col h-full bg-[#F5F6F7]">
      <CardHeader className="p-0 shrink-0 bg-white border-b border-black/[0.08]">
        <div className="p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><CardTitle className="text-base font-black text-accent">{botName}</CardTitle><p className="text-[10px] text-accent/40 font-black uppercase tracking-widest">{botSub}</p></div>
          </div>
          <Button variant="ghost" size="icon" onClick={onClose} className="text-muted-foreground hover:bg-black/5 rounded-full">{isExpanded ? <X className="w-5 h-5" /> : <Maximize2 className="w-5 h-5" />}</Button>
        </div>
      ...
      <CardContent ref={scrollRef} className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        {isLoading && (<div className="flex items-start gap-3 animate-pulse"><AvatarIcon src={botIconUrl} avatarId={activeBot} className="w-8 h-8" /><div className="bg-white border border-black/[0.08] p-3.5 rounded-2xl rounded-tl-none text-xs font-bold text-muted-foreground">답변을 준비하고 있습니다...</div></div>)}
      </CardContent>
      <div className="p-4 bg-white border-t border-black/[0.08]">
        <div className="relative flex items-center gap-2">
          <Input placeholder="궁금한 내용을 질문해 보세요" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} disabled={isLoading} className="border-black/10 focus-visible:ring-accent h-12 text-sm font-bold rounded-xl placeholder:text-black/10 shadow-sm" />
          <Button size="icon" onClick={handleSend} disabled={!input.trim() || isLoading} className="naver-button h-12 w-12 shrink-0 shadow-lg rounded-xl active:scale-95"><Send className="w-4 h-4" /></Button>
        </div>
      </div>
    </div>
  )
...
export function AldiChat({ forceOpenTrigger, onTriggerClose, hideCard = false }: any) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setConversations(prev => ({ ...prev, [currentBot]: [...prev[currentBot], { role: "bot", text: "일시적인 오류가 발생했습니다." }] }))
    } finally { setIsLoading(false) }
  }

  return (
    <>
      {!hideCard && (<Card className="naver-card flex flex-col h-[500px] overflow-hidden shadow-2xl border-primary/20"><ChatInterface messages={conversations[activeBot]} input={input} setInput={setInput} isLoading={isLoading} handleSend={handleSend} activeBot={activeBot} onBotChange={setActiveBot} onClose={() => setIsFocused(true)} botConfig={botConfig} /></Card>)}
      <Dialog open={isFocused} onOpenChange={setIsFocused}><DialogContent className="max-w-2xl h-[85vh] p-0 border-none overflow-hidden rounded-[2rem] shadow-3xl"><DialogHeader className="sr-only"><DialogTitle>AI 전문가 상담</DialogTitle></DialogHeader><ChatInterface messages={conversations[activeBot]} input={input} setInput={setInput} isLoading={isLoading} handleSend={handleSend} isExpanded activeBot={activeBot} onBotChange={setActiveBot} onClose={() => setIsFocused(false)} botConfig={botConfig} /></DialogContent></Dialog>
    </>
  )
}
