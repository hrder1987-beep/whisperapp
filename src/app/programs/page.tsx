...
export default function ProgramsPage() {
  ...
  const [selectedType, setSelectedType] = useState<"all" | "program" | "solution">("all")
                ...
                <div className="flex-1 overflow-y-auto">
                  <form onSubmit={handleAddProgram} className="p-10 space-y-12 pb-24">
                    ...
                    <div className="space-y-4">
                      ...
                      <Tabs value={contentType} onValueChange={(v) => setContentType(v as any)} className="w-full">
                        <TabsList className="grid grid-cols-2 h-14 bg-[#F5F6F7] p-1 rounded-2xl">
                          <TabsTrigger value="program" className="rounded-xl font-black gap-2 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-md">
                            <GraduationCap className="w-4 h-4" /> 교육 프로그램 (과정)
                          </TabsTrigger>
                          <TabsTrigger value="solution" className="rounded-xl font-black gap-2 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-md">
                    ...
                    <Button type="submit" disabled={isSubmitting} className="w-full h-16 naver-button text-lg rounded-xl shadow-2xl hover:scale-[1.01] transition-all">
                      {isSubmitting ? "정보 등록 중..." : `${contentType === 'program' ? '교육 과정' : '솔루션'} 정보 게시 완료`}
                    </Button>
                  </form>
                </div>
              </DialogContent>
              ...
              <Card key={p.id} className="group cursor-pointer flex flex-col h-full bg-white border border-black/[0.06] hover:border-primary/20 hover:shadow-2xl rounded-[2.5rem] overflow-hidden transition-all duration-500">
                <div className="relative aspect-[16/10] w-full overflow-hidden bg-black/5" onClick={() => setSelectedProgram(p)}>
                  ...
                  <div className="absolute top-5 left-5 flex flex-col gap-2">
                    ...
                    <Badge className="bg-white/90 backdrop-blur-sm text-black/60 font-black border-none px-4 py-1.5 rounded-full text-[9px] shadow-sm">#{PROGRAM_CATEGORIES.find(c => c.id === p.category)?.name || "기타"}</Badge>
                  </div>
                  {p.cost && (
