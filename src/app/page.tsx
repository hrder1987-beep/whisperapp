...
const MOCK_QUESTIONS: Question[] = [
  { id: "sample-1", title: "휴일에 근무하면 무조건 보상휴가로 처리해야 하나요?", text: "저희 팀원이 이번 주말에 나와서 일하게 됐는데, 이걸 꼭 보상휴가로만 줘야 하는 건지 궁금해요. 대체휴일로 운영해도 법적으로 문제가 없을까요? 실무자분들은 어떻게 하시나요?", nickname: "초보인사돌이", userId: "mock-1", userRole: "member", jobTitle: "인사담당자", viewCount: 142, answerCount: 1, createdAt: Date.now() - 1000 * 60 * 60 * 1, category: "인사전략/HRM" },
  ...
  { id: "sample-35", title: "1년 미만자에게 연차를 선부여하는 제도, 불법은 아니죠?", text: "저희 회사는 신입사원분들 기 살려주려고 입사하자마자 연차를 미리 며칠 주려고 하거든요. 이게 법적으로 문제가 되는 선부여 방식인가요? 오히려 유리한 거니까 괜찮겠죠?", nickname: "연차계산기", userId: "mock-35", userRole: "member", jobTitle: "인사운영", viewCount: 215, answerCount: 1, createdAt: Date.now() - 1000 * 60 * 60 * 50, category: "복지/유연근무" },
      ...
      <div className="max-w-7xl mx-auto px-0 md:px-4 py-0 md:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <main className={cn(
            ...
            searchQuery.trim().length > 0 ? "lg:col-span-12 max-w-4xl mx-auto w-full" : "lg:col-span-8"
          )}>
            {searchQuery.trim().length > 0 ? (
              <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 px-4 md:px-0 mt-8">
                <div className="flex flex-col gap-6 mb-12">
                  <button onClick={() => setSearchQuery("")} className="flex items-center gap-2 text-primary/40 hover:text-accent font-bold text-sm transition-colors w-fit group">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-lg font-bold text-primary/30">{filteredQuestions.length}개의 HR 집단지성을 찾았습니다.</p>
                  </div>
                </div>
                ...
                {totalPages > 1 && (
                  <div className="flex justify-center items-center gap-2 mt-12 pb-10">
                    <Button 
                      ...
                      className="rounded-xl text-primary/40"
                    >
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                      <Button
                        ...
                        className={cn(
                          ...
                          currentPage === page ? "bg-primary text-accent shadow-lg" : "bg-white text-primary/20 hover:bg-primary/5 shadow-sm"
                        )}
                      >
                        {page}
                      </Button>
