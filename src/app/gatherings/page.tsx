          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button className="naver-button h-14 px-10 rounded-xl shadow-xl gap-3 text-base text-accent"><Plus className="w-5 h-5" /> 신규 모임 만들기</Button>
              </DialogTrigger>
                ...
                <div className="flex-1 overflow-y-auto p-8 md:p-10">
                  <form onSubmit={handleCreateGathering} className="space-y-8">
                    <div className="space-y-4">
                      ...
                      <div onClick={() => fileInputRef.current?.click()} className="relative aspect-video bg-[#F5F6F7] border-2 border-dashed border-accent/10 rounded-2xl flex items-center justify-center cursor-pointer group hover:border-primary overflow-hidden transition-all shadow-inner">
                    ...
                    <Button type="submit" disabled={isSubmitting} className="w-full h-16 naver-button text-lg rounded-2xl shadow-2xl mt-4">{isSubmitting ? "생성 중..." : "모임 개설 완료"}</Button>
                  </form>
                </div>
              </DialogContent>
            </Dialog>
