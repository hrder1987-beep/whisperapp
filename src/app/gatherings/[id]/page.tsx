      ...
      <main className="max-w-7xl mx-auto px-4 -mt-20 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <Card className="bg-white border-none shadow-2xl rounded-[2.5rem] overflow-hidden">
              <CardContent className="p-8 md:p-12">
                <div className="flex flex-wrap gap-2 mb-6">
                  <Badge className="bg-accent text-primary font-black border-none px-4 py-1.5 rounded-full text-[10px]">#{gathering.category}</Badge>
                  <Badge className={cn("font-black border-none px-4 py-1.5 rounded-full text-[10px]", gathering.status === 'recruiting' ? "bg-emerald-500 text-white" : "bg-primary/10 text-primary/40")}>
                    {gathering.status === 'recruiting' ? "모집 중" : "모집 완료"}
                  </Badge>
                  <Badge className="bg-primary text-accent font-black border-none px-4 py-1.5 rounded-full text-[10px]">{gathering.sessionCount}회차 정기 모임</Badge>
                </div>
                
                <h1 className="text-3xl md:text-4xl font-black text-primary tracking-tighter mb-8 leading-tight">{gathering.title}</h1>
                
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  <TabsList className="bg-primary/5 p-1 rounded-2xl h-14 w-full md:w-fit grid grid-cols-2 md:flex mb-10 overflow-x-auto scrollbar-hide">
                    <TabsTrigger value="overview" className="rounded-xl font-black text-xs md:px-8 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-lg">상세 소개</TabsTrigger>
                    <TabsTrigger value="sessions" className="rounded-xl font-black text-xs md:px-8 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-lg">
                      세션 및 출석 {(isApproved || isCreator) ? "" : "🔒"}
                    </TabsTrigger>
                    <TabsTrigger value="lms" className="rounded-xl font-black text-xs md:px-8 data-[state=active]:bg-white data-[state=active]:text-primary data-[state=active]:shadow-lg">
                      학습 자료실 {(isApproved || isCreator) ? "" : "🔒"}
                    </TabsTrigger>
                    {isCreator && (
                      <TabsTrigger value="admin" className="rounded-xl font-black text-xs md:px-8 data-[state=active]:bg-primary data-[state=active]:text-accent data-[state=active]:shadow-lg gap-2">
                  ...
                  <TabsContent value="overview" className="mt-0">
                    <div className="prose prose-slate max-w-none">
                      <p className="text-[16px] leading-relaxed text-primary/70 whitespace-pre-wrap font-medium">{gathering.description}</p>
                    </div>
                  </TabsContent>
                    ...
                    {(isApproved || isCreator) ? (
                      <div className="space-y-8">
                        <div className="bg-accent/5 border border-accent/20 p-6 rounded-2xl">
                          <h4 className="font-black text-primary text-lg mb-1">회차별 출석 체크</h4>
                          <p className="text-xs font-bold text-primary/40">각 회차별 모임 참석 여부를 제출해 주세요.</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          {sessionArray.map(sessionNum => {
                            ...
                            return (
                              <div key={sessionNum} className="p-6 bg-white border border-primary/5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 group hover:border-accent transition-all">
                                <div className="flex items-center gap-4">
                                  ...
                                  <div>
                                    <p className="font-black text-primary">{sessionNum}회차 정기 세션</p>
                                    <p className="text-xs font-bold text-primary/30">일정은 개설자 안내를 참조하세요.</p>
                                  </div>
                                </div>
                      ...
                      <div className="py-24 text-center bg-primary/5 rounded-[2.5rem] space-y-6">
                        <Users className="w-16 h-16 text-primary/10 mx-auto" />
                        <div className="space-y-2 px-6">
                          <h3 className="text-xl font-black text-primary">승인된 멤버 전용 공간입니다.</h3>
                          <p className="text-sm font-bold text-primary/40">모임 참여 승인 후 매월 출석 체크를 하실 수 있습니다.</p>
                        </div>
                  ...
                  <TabsContent value="lms" className="mt-0">
                    {(isApproved || isCreator) ? (
                      <div className="space-y-6">
                        <div className="bg-accent/5 border border-accent/20 p-6 rounded-2xl flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <div className="p-2 bg-accent text-primary rounded-xl"><FileText className="w-5 h-5" /></div>
                            <div>
                              <h4 className="font-black text-primary text-lg">회차별 학습 자료실</h4>
                              <p className="text-xs font-bold text-primary/40">모임 진행 시 활용되는 각종 템플릿과 강의안이 공유됩니다.</p>
                            </div>
                          </div>
                        </div>
                              ...
                              <div key={i} className="p-6 bg-white border border-primary/5 rounded-2xl flex items-center justify-between group hover:border-accent transition-all">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div className="min-w-0">
                                    <span className="font-bold text-primary truncate block">{res.title}</span>
                                    {res.sessionId && <Badge className="bg-primary/5 text-primary/40 font-black text-[9px] border-none px-1.5 h-4 mt-1">{res.sessionId}회차 자료</Badge>}
                                  </div>
                                </div>
                                <Button variant="ghost" size="icon" className="text-primary/20 hover:text-accent shrink-0"><Download className="w-5 h-5" /></Button>
                              </div>
                            ))}
                          </div>
                        )}
                      ...
                      <div className="py-24 text-center bg-primary/5 rounded-[2.5rem] space-y-6">
                        <ShieldCheck className="w-16 h-16 text-primary/10 mx-auto" />
                        <div className="space-y-2 px-6">
                          <h3 className="text-xl font-black text-primary">참여자 전용 자료실입니다.</h3>
                          <p className="text-sm font-bold text-primary/40">모임 신청 후 승인이 완료되면 회차별 자료를 보실 수 있습니다.</p>
                        </div>
                        <Button onClick={handleApply} disabled={!!myApp} className="bg-primary text-accent font-black h-12 px-10 rounded-xl">
                          {myApp ? "승인 대기 중" : "지금 바로 신청하기"}
                        </Button>
                        ...
                        {(!applications || applications.length === 0) ? (
                          ...
                          <div className="divide-y divide-primary/5">
                            {applications.map(app => (
                              <div key={app.id} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-white transition-all">
                                <div className="flex items-center gap-4">
                                  <div className="w-10 h-10 rounded-xl bg-primary text-accent flex items-center justify-center font-black">
                                    {app.userName.substring(0, 1)}
                                  </div>
                                  <div>
                                    <p className="font-black text-primary">@{app.userName}</p>
                                    <p className="text-[10px] font-bold text-primary/30 flex items-center gap-1"><Clock className="w-3 h-3" /> {formatDistanceToNow(app.appliedAt, { addSuffix: true, locale: ko })} 신청</p>
                                  </div>
                                </div>
                                ...
                                <div className="flex items-center gap-3">
                                  {app.status === 'pending' ? (
                                    <>
                                      <Button onClick={() => handleApprove(app)} className="bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-xl h-10 px-5 gap-2 text-xs">
                                        <Check className="w-4 h-4" /> 승인
                                      </Button>
                                      <Button onClick={() => handleReject(app)} variant="outline" className="border-red-100 text-red-500 hover:bg-red-50 font-black rounded-xl h-10 px-5 gap-2 text-xs">
                                        <X className="w-4 h-4" /> 반려
                                      </Button>
                                    </>
                                  ) : (
                                    <Badge className={cn("font-black px-4 py-1.5 rounded-lg border-none text-[10px]", app.status === 'approved' ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-500")}>
          ...
          <aside className="lg:col-span-4 space-y-8">
            <Card className="bg-white border-none shadow-2xl rounded-[2.5rem] p-8 space-y-8 sticky top-32">
              <h4 className="text-lg font-black text-primary border-b border-primary/5 pb-4">모임 요약 정보</h4>
              
              <div className="space-y-6">
                <div className="flex flex-col gap-1.5">
                  <span className="text-[10px] font-black text-primary/30 uppercase tracking-widest">전체 모임 일정</span>
                  <div className="flex items-center gap-3 font-bold text-primary/70 text-sm">
                ...
                <div className="flex flex-col gap-1.5">
                  <span className="text-[10px] font-black text-primary/30 uppercase tracking-widest">회차 정보</span>
                  <div className="flex items-center gap-3 font-bold text-primary/70 text-sm">
                    <div className="p-2 bg-primary/5 rounded-lg"><Clock className="w-4 h-4 text-accent" /></div>
                    총 {gathering.sessionCount}회차 정기 세션
                  </div>
                </div>

                <div className="flex flex-col gap-1.5">
                  <span className="text-[10px] font-black text-primary/30 uppercase tracking-widest">장소</span>
                  <div className="flex items-center gap-3 font-bold text-primary/70 text-sm">
                ...
                <div className="flex flex-col gap-3">
                  <span className="text-[10px] font-black text-primary/30 uppercase tracking-widest">모집 인원 ({gathering.participantCount}/{gathering.capacity})</span>
              ...
              {!isCreator && (
                <div className="pt-4">
                  {myApp ? (
                    <div className={cn(
                      "w-full h-14 rounded-2xl flex items-center justify-center font-black text-base gap-3",
                    ...
                    <Button 
                      ...
                      className="w-full h-16 bg-primary text-accent hover:bg-primary/95 font-black text-lg rounded-2xl shadow-xl gap-3"
                    >
              ...
              <div className="pt-8 border-t border-primary/5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-primary/30 uppercase">모임 호스트</p>
