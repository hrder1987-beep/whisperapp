...
import { Plus, Star, Award, Briefcase, MessageSquare, Crown, Camera, X, Sparkles, Search } from "lucide-react"
          ...
          <div className="space-y-4 flex-1">
            ...
            <div className="relative max-w-xl group pt-4">
              ...
              <Input 
                ...
                placeholder="전문 위스퍼러의 성함이나 분야를 검색하세요..." 
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="max-w-xl bg-white border-none rounded-[3rem] p-10 shadow-2xl">
              ...
              <form onSubmit={handleAddMentor} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-primary/40 ml-2">주요 전문 분야</label>
              ...
              <Card key={m.id} className="group bg-white border-primary/5 rounded-[3rem] overflow-hidden shadow-sm hover:shadow-2xl hover:-translate-y-3 transition-all duration-500">
                <CardContent className="p-10 flex flex-col items-center text-center">
                  <div className="relative mb-8">
                    ...
                    <Badge className="absolute -bottom-3 right-0 bg-primary text-accent font-black border-none px-4 py-1.5 rounded-xl flex gap-1.5 items-center animate-bounce shadow-xl">
                      <Award className="w-3.5 h-3.5" /> WHISPERER
                    </Badge>
                  </div>
                  
                  <h3 className="text-2xl font-black text-primary mb-1.5 group-hover:text-accent transition-colors">{m.name}</h3>
                  <p className="text-accent font-black text-xs uppercase tracking-widest mb-6">#{m.specialty}</p>
                  
                  <div className="w-14 h-1 bg-primary/5 mb-8 rounded-full group-hover:w-24 group-hover:bg-accent transition-all duration-500"></div>
                  
                  <p className="text-sm text-primary/50 line-clamp-4 mb-10 font-medium leading-relaxed italic px-2">
                    "{m.bio}"
                  </p>

                  <div className="grid grid-cols-1 w-full gap-4 mt-auto">
                    <Button variant="outline" className="h-14 rounded-2xl border-primary/10 text-primary font-black gap-2 hover:bg-primary hover:text-accent transition-all">
                      <MessageSquare className="w-5 h-5" /> 1:1 인사이트 문의
                    </Button>
                  </div>
                </CardContent>
              </Card>
